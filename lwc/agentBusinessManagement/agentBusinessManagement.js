...
export default class AgentBusinessManagement extends NavigationMixin(LightningElement) {
    /**
     * @private
...
//Termina lo agregado por Labs
    ...
    async handlerButtonLarge(event) {
            ...
            switch (options.option) {
                ...
                default:
                    ...
                    if(this.isValidCompany) {
                        this.showMenu = true;
                        if(optionId=='SegurosGenerales/VidaNoULCotizaronlineAutosAutosLineasPersonales'){
                            omniHogarShow = true;
                        }else if(optionId=='SegurosGenerales/VidaNoULBuscarPresup.yÓrdenesOnlineAutosAutosLineasPersonales'){
                            
                            listQuotesShow = true;
                            var urlCom1 = window.location.origin;
                            var urlString = urlCom1 + '/zurichagentes/s/cotizaciones';
                            console.log('urlCom1 '+urlString);
                            ...
                            this.showIframe = true;
                            this.showIframeCotizador = false;
                        }
                    }else{
                        this.showIframe = false;
                        //Agregado por Labs
                        omniHogarShow = false;
                        listQuotesShow = false;
                    }
                    break;
            }
        } else {
        ...
        //Agregado por Labs
        const sendDataEvent = new CustomEvent('senddata', {
            /* esta linea se dejara comentada en caso de aceptar la por la 2da op de la vista
            Nota: se debera tener en cuenta la modificacion en el componente aura y en el controlador del mismo */
            //detail: {omniShow:omniHogarShow,QuotesShow:listQuotesShow} 
            detail: {omniHogarShow}
        });
        this.dispatchEvent(sendDataEvent);
